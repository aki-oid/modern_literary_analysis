...
# ===== config =====
FILE_02_1 = "data/02-1b_clustering_results.csv" # 構造(Shape)
FILE_02_2 = "data/02-2_features_style.csv"     # スタイル(形態素)
FILE_02_3 = "data/02-3_features_topics.csv"      # 内容(トピック)
OUTPUT_MASTER = "data/02_master_dataset.csv"

def load_and_merge():
    print("各分析データを読み込み中...")
    
    # 1. 各データの読み込み
    df_shape = pd.read_csv(FILE_02_1)
    df_morph = pd.read_csv(FILE_02_2)
    df_topic = pd.read_csv(FILE_02_3)
...
if __name__ == "__main__":
    if all(os.path.exists(f) for f in [FILE_02_1, FILE_02_2, FILE_02_3]):
        load_and_merge()
    else:
